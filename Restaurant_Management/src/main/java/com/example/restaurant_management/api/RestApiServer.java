...
public class RestApiServer {
    ...
    public void start() throws IOException {
            ...
            try {
                ...
                if (attempt == maxAttempts - 1) {
                    throw new IOException("Failed to start server after " + maxAttempts + " attempts. " +
                        ...
                        "Please close the application using port " + DEFAULT_PORT + " or restart your computer.", e);
                }
                port++;
        ...
        // Dynamic order routes - GET /api/orders/{id}, POST /api/orders/{id}/items, POST /api/orders/{id}/pay
    ...
    private void handlePostOrder(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            Integer tableId = jsonRequest.has("tableId") && !jsonRequest.get("tableId").isJsonNull() 
                ? jsonRequest.get("tableId").getAsInt() : null;
    ...
    private void handleOrderRoutes(HttpExchange exchange) throws IOException {
        ...
        if (parts.length >= 5 && "items".equals(parts[parts.length - 1]) && "POST".equals(exchange.getRequestMethod())) {
    ...
    private void handleAddItemsToOrder(HttpExchange exchange, String[] parts) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < itemsArray.size(); i++) {
                ...
                if (food != null && "CON_HANG".equals(food.getStatus())) {
                    ...
                    for (OrderDetail existing : existingDetails) {
                        if (existing.getMonId() == foodId) {
                            // Update quantity
                            existing.setSoLuong(existing.getSoLuong() + quantity);
                            existing.setThanhTien(existing.getDonGia() * existing.getSoLuong());
                            try {
                                orderDetailRepo.updateOrderDetail(existing);
                            } catch (java.sql.SQLException e) {
                                throw new RuntimeException("Failed to update order detail: " + e.getMessage(), e);
                            }
                            itemExists = true;
                            additionalTotal += food.getPrice() * quantity;
                            break;
                        }
                    }
            ...
            // Recalculate total
            List<OrderDetail> allDetails = orderDetailRepo.findByOrderId(orderId);
            double newTotal = 0.0;
            for (OrderDetail detail : allDetails) {
                newTotal += detail.getThanhTien();
            }
            
            // Update order total
            try {
                orderRepo.updateOrderTotal(orderId, newTotal);
            } catch (java.sql.SQLException e) {
            ...
            response.addProperty("orderId", orderId);
            response.addProperty("newTotal", newTotal);
            response.addProperty("additionalTotal", additionalTotal);
    ...
    private void handlePayOrder(HttpExchange exchange, String[] parts) throws IOException {
        try {
            ...
            double amountPaid = jsonRequest.has("amountPaid") && !jsonRequest.get("amountPaid").isJsonNull()
                ? jsonRequest.get("amountPaid").getAsDouble() : 0.0;
            String note = null;
            ...
            // For now, we'll update the order status and save payment info to a payments table
            // First, let's create the payment record
            try {
                java.sql.Connection conn = com.example.restaurant_management.ConnectDB.ConnectDB.getConnection();
                
                // Create payment record
                String paymentSql = "INSERT INTO payments (order_id, payment_method, amount_paid, note, paid_at) VALUES (?, ?, ?, ?, ?)";
